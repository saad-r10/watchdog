    ...
    <div className="bg-slate-900 rounded-xl border border-slate-800 p-5">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Security Headers</h3>
        </div>
        {data && (
      ...
      {isLoading ? (
        ...
        <p className="text-sm text-slate-500">Header check runs every 6 hours — check back soon.</p>
      ) : (
          ...
          {SECURITY_HEADERS.map((h) => {
            ...
            return (
              <div key={h} className="flex items-center justify-between py-1">
                <span className={`text-xs font-mono ${isPresent ? "text-slate-300" : "text-slate-600"}`}>
                  {HEADER_LABELS[h] ?? h}
                </span>
                {isPresent ? (
