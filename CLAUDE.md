...
| `POST /api/agents` | JWT | Create agent (returns one-time key); body accepts an optional free-text `region` label |
| `PATCH /api/agents/:id` | JWT | Rename an agent or update its `region` label |
...
Multiple agents — each optionally tagged with a free-text `region` (e.g. `us-east`, `eu-west`) — can be assigned to the same monitor via a `MonitorAgent` join table (replaces the old 1:1 `Monitor.agentId`). Every `Check` is stamped with its source `agentId` (`NULL` = Watchdog's cloud checker).

| Endpoint | Auth | Purpose |
|----------|------|---------|
| `POST /api/monitors/:id/agents/:agentId` | JWT | Assign an agent (region) to a monitor |
| `DELETE /api/monitors/:id/agents/:agentId` | JWT | Unassign an agent from a monitor |
| `GET /api/monitors/:id/regions` | JWT | Per-source latest status (`agentId`, `label`, `region`, `status`, `statusCode`, `responseTime`, `checkedAt`) — one row per assigned agent, or a single `"Cloud"` row if none are assigned |

`Monitor.regionDownThreshold` (default `1`, settable via `PATCH /api/monitors/:id`, capped at the number of assigned sources) controls how many distinct sources must report `down` simultaneously before `monitorStatusService.evaluateUptimeStatus` opens a `downtime` incident; it resolves once the down-count drops back below the threshold. With 0 or 1 assigned agents this is identical to the original single-source behavior. The monitor detail page's "Regions" card surfaces per-source status and (when 2+ agents are assigned) the threshold selector.
...
- **Channels**: email via Nodemailer (SMTP), generic webhook (HTTP POST), Slack incoming webhook, Discord server webhook, and Telegram bot — all fired in parallel via `Promise.allSettled` so one channel's failure never blocks the others
- **Triggers**: downtime (open/resolve threshold configurable per monitor across regions — see "Multi-Region Agents"), SSL expiry within 14 days, missing critical headers, unexpected certificate detected via Certificate Transparency logs, monitored domain detected on a threat-intel blocklist (URLhaus/Spamhaus DBL), unexpected page content change (opt-in defacement detection), response-time anomaly vs. rolling 7-day baseline (`performance_degraded`), Lighthouse performance/accessibility/best-practices/SEO score below per-monitor budget (`lighthouse_budget_exceeded`)
- **Chat channels**: `slackWebhookUrl` (Slack Incoming Webhook URL), `discordWebhookUrl` (Discord server webhook URL), `telegramBotToken` + `telegramChatId` (Telegram Bot API). Each has a test endpoint: `POST /api/users/me/settings/test-slack`, `test-discord`, `test-telegram`. Formatters live in `slack.service.ts`, `discord.service.ts`, `telegram.service.ts`
- **Web Push (VAPID)**: `alertWebPush` boolean on `User` enables browser push notifications via the W3C Push API. Subscriptions are stored in the `PushSubscription` model (one row per browser). Routes: `GET /api/users/me/settings/vapid-public-key` (public key for subscribe call), `POST /api/users/me/settings/push-subscription` (save subscription), `DELETE /api/users/me/settings/push-subscription` (unsubscribe), `POST /api/users/me/settings/test-push`. Service worker at `apps/frontend/public/sw.js` handles the `push` event. Requires `VAPID_PUBLIC_KEY`, `VAPID_PRIVATE_KEY`, `VAPID_SUBJECT` env vars; no-ops silently if unset
- **Webhook**: Payload: `{ event, monitorId, monitorName, monitorUrl, incidentId, startedAt }`. Test endpoint: `POST /api/users/me/settings/test-webhook`
...
| `MonitorAgent` | Join table assigning one or more Agents (regions) to a Monitor (many-to-many) |
| `Check` | Single uptime/ssl/header/cert_transparency/dns/exposure/blocklist/lighthouse result — uptime checks carry optional phase timings (`dnsMs`, `tcpMs`, `tlsMs`, `ttfbMs`, `downloadMs`), `sizeBytes`, `agentId` (source agent, `NULL` = cloud checker), and (when `Monitor.contentChangeEnabled`) `contentHash`, a SHA-256 hex digest of the response body used for defacement detection; header checks carry `headers` (JSON: present/missing security headers, `cookies` Set-Cookie attribute findings, and `mixedContent` HTTP-on-HTTPS findings); cert_transparency checks carry `ctNewCerts` (JSON list of newly-seen certs); dns checks carry `dnsFindings` (JSON: SPF/DMARC/DKIM/dangling-CNAME results); exposure checks carry `exposureFindings` (JSON: security.txt presence and exposed-path results); blocklist checks carry `blocklistFindings` (JSON: per-source listed/clean results from URLhaus and Spamhaus DBL); lighthouse checks carry `lighthouseResult` (JSON: `{success, performance, accessibility, bestPractices, seo, error?}` — 0-100 category scores) |
| `Incident` | Downtime, SSL-expiry, header, unexpected-certificate, domain-blocklisted, content-changed, performance-degraded, or lighthouse-budget-exceeded event |
